...
def train_val_data_process():
    # 定义数据集的路径
    ROOT_TRAIN = 'gdgoc-telkom-university-2026-mask-classification/Dataset/train'
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    for epoch in tqdm(range(num_epochs), desc="Epoch", position=0):
        ...
        for step, (b_x, b_y) in tqdm(enumerate(train_dataloader), desc="train_batch", position=1, leave=False):
            b_x = b_x.to(device)
            b_y = b_y.to(device)
        ...
        for step, (b_x, b_y) in tqdm(enumerate(val_dataloader), desc="val_batch", position=1, leave=False):
            b_x = b_x.to(device)
            b_y = b_y.to(device)
        ...
        tqdm.write(('{} Train Loss: {:.4f} Train Acc: {:.4f}'.format(epoch, train_loss_all[-1], train_acc_all[-1])))
        tqdm.write('{} Val Loss: {:.4f} Val Acc: {:.4f}'.format(epoch, val_loss_all[-1], val_acc_all[-1]))
